  ...
  async run(commandOptions, rawArgs) {
    ...
    if (!commandOptions.skipNpm) {
      if (commandOptions.packageManager) {
        if (commandOptions.packageManager === 'npm') {
          await execa('npm', ['install']);
        } else if (commandOptions.packageManager === 'pnpm') {
          await execa('pnpm', ['install']);
        } else if (commandOptions.packageManager === 'yarn') {
          await execa('yarn', ['install']);
        }
      } else if (await isPnpmProject(project.root)) {
        await execa('pnpm', ['install']);
      } else if (isYarnProject(project.root)) {
        await execa('yarn', ['install']);
      } else {
